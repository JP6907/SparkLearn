...
import SparkStreaming.KafkaDirectWordCount3.MyZKGroupTopicDirs
import SparkStreaming.Order_MultiIndex.JedisConnectionPool
...
object KafkaDirectWordCount4 {
  def main(args: Array[String]): Unit = {
    ...
    kafkaStream.foreachRDD{kafkaRDD =>
      if(!kafkaRDD.isEmpty()){
        //只有kafkaRDD可以强制转成HasOffsetRanges，并获取偏移量
