...
import SparkStreaming.StreamingWordCount.updateFunc
...
/**
  * zookeeper保存偏移量
  * 本地保存累加历史结果
  * 操作数据需要用到kafkaStream.transform
  *
  * 这种方式程序做了checkpoint保存累加历史数据
  * 但是程序重启之后不会读取保存的数据，
  * 而不会使用保存的累加结果，
  * 以0开始从zookeeper中保存的偏移量开始计算
  */
object KafkaDirectWordCount3 {

  class MyZKGroupTopicDirs(group: String, topic: String) extends ZKGroupTopicDirs(group: String, topic: String) with Serializable{}

  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setAppName("KafkaDirectWordCount").setMaster("local[2]")
    val ssc = new StreamingContext(conf,Duration(5000))

    ssc.checkpoint("E:\\SparkLearn\\src\\main\\resources\\wc_ck4")
    ...
    val topicDirs = new MyZKGroupTopicDirs(groupId,topic)
    ...
    /**
      * 处理数据
      */
    val words : DStream[String] = messages.flatMap(_.split(" "))
    ...
    messages.foreachRDD{rdd =>
      ...
      for(o <- offsetRanges){
        ...
        val zkClient1 = new ZkClient(zkQuorum)  //不是ZKClient，k为小写
        ZkUtils.updatePersistentPath(zkClient1,zkPath,o.untilOffset.toString)
      }
    }
